...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            // 0 EarPhoneMode
            longClickItemEarPhoneMode(alertDialog, msgType);    //  语音切换播放模式
            // 1 resend
            longClickItemResend(selectedItem, alertDialog);     //  发送失败，重发
            // 2 copy
            longClickItemCopy(selectedItem, alertDialog, msgType);  //  文字复制
            // 3 revoke
            if (enableRevokeButton(selectedItem)) {
//                longClickRevokeMsg(selectedItem, alertDialog);    //  撤回消息
            }
            // 4 delete
            longClickItemDelete(selectedItem, alertDialog);     //  删除消息
            // 5 trans
...
//                longClickItemForwardToTeam(selectedItem, alertDialog);
        ...
        private void longClickItemForwardToPerson(final IMMessage item, CustomAlertDialog alertDialog) {
            alertDialog.addItem(container.activity.getString(R.string.forward), new CustomAlertDialog.onSeparateItemClickListener() {
...
//                    option.maxSelectNum = 1;
                    if (NimUIKit.getContactSelectorHelper() != null) {
                        ContactSelectOption options = NimUIKit.getContactSelectorHelper().getOptions();
                        MsgAttachment attachment = item.getAttachment();
                        options.setAttachment(attachment);
                        NimUIKit.startContactSelector(container.activity,
                                options, REQUEST_CODE_FORWARD_PERSON);
                    }
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        final String selected = data.getStringExtra("RESULT_DATA");
        if (TextUtils.isEmpty(selected)) {
            return;
        }
        switch (requestCode) {
            case REQUEST_CODE_FORWARD_TEAM:
                doForwardMessage(selected, SessionTypeEnum.Team);
                break;
            case REQUEST_CODE_FORWARD_PERSON:
                doForwardMessage(selected, SessionTypeEnum.P2P);
                break;
        }
