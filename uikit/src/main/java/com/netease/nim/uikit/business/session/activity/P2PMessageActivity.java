...
public class P2PMessageActivity extends BaseMessageActivity {
    ...
    public static void start(Context context, String contactId, SessionCustomization customization, IMMessage anchor,
                             int keepTime, String queryId, String consultantId) {
        ...
        intent.putExtra(Extras.EXTRA_KEEP_TIME, keepTime);
        if (!TextUtils.isEmpty(queryId)) {
            intent.putExtra(Extras.EXTRA_QUERY_ID, queryId);
        }
        if (!TextUtils.isEmpty(consultantId)) {
            intent.putExtra(Extras.EXTRA_CONSULTANT_ID, consultantId);
        }
...
//                ToastHelper.showToast(P2PMessageActivity.this, "command: " + content);
    ...
    protected void initToolBar() {
        ...
        setToolBar(R.id.top_bar, options);
    }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        for (int grantResult : grantResults) {
            if (grantResult != PackageManager.PERMISSION_GRANTED) {
                showDeniedDialog("您禁止了拨打录音权限，是否现在去开启？");
            }
        }
    }
