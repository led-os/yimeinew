...
public class BannerViewPager extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private int seleteid = R.drawable.yuan_blue;
    private int unseleteid = R.drawable.yuan_bai;
    ...
    private void init(Context context) {
        ...
        View view = View.inflate(context, R.layout.view_scrollbanner, this);
...
//        addView(view);
        ...
        try {
            ...
            FixedSpeedScroller scroller = new FixedSpeedScroller(vp_banner.getContext(),
                    new AccelerateInterpolator());
            field.set(vp_banner, scroller);
            scroller.setmDuration(1800);
        } catch (Exception e) {
    ...
    public void setindicatorColor(@DrawableRes int unseleid, @DrawableRes int seleteid) {
        this.unseleteid = unseleid;
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].setBackgroundResource(unseleteid);
        }
        if (indicators.length > selectedPosition)
            indicators[selectedPosition]
                    .setBackgroundResource(seleteid);
    }
...
//            Glide.with(getContext()).load(list.get(position)).into(v);
    ...
    protected void onDetachedFromWindow() {
        ...
        handler.removeMessages(LOOP_WAIT);
    }
}
