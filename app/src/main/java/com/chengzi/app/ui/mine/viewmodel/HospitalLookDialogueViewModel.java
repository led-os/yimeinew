...
public class HospitalLookDialogueViewModel extends BaseViewModel implements IRequest {
    ...
    public HospitalLookDialogueViewModel() {
    ...
    public void onRequest(int currentPage, int pageSize) {
        mineService.lookSheetNum(AccountHelper.getToken(), AccountHelper.getUserId(), user_type.get(),
                String.valueOf(currentPage), String.valueOf(pageSize))
                .subscribe(new SimpleObserver<ResponseBean<PageBean<LookSheetNumBean>>>() {
                    @Override
                    public void onSuccess(ResponseBean<PageBean<LookSheetNumBean>> responseBean) {
                        lookSheetNumLiveData.postValue(responseBean.getData().getData());
                    }

                    @Override
                    public void onError(int code, String errorMsg) {
                        super.onError(code, errorMsg);
                        lookSheetNumLiveData.postValue(new ArrayList<>());
                    }
                });
    }
}